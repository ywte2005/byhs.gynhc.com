define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'sortable'], function ($, undefined, Backend, Table, Form, Sortable) {
    var Controller = {
        index: function () {
            ...
            //初始化菜单
            $("#menu-list").prepend(Template("menutpl", {menus: menus}));
            ...
            $(document).on('click', "#menuSave", function () {
                ...
                let v = $(".n-msg").text();
                if (v){
                    Toastr.error('表单验证未通过：' + v)
                } else {
                    $("#menuSave").addClass('disabled');
                    $.post("weixin/menus/save", {menus: JSON.stringify(getMenuList())}, function (ret) {
                        var msg = ret.hasOwnProperty("msg") && ret.msg != "" ? ret.msg : "";
                        if (ret.code == 1) {
                            Backend.api.toastr.success(ret.msg);
                            setTimeout("location.reload()", 1500); // 刷新
                        } else {
            ...
            //点击同步
            $(document).on('click', "#menuSaveSyn", function () {
                //检查表单是否通过验证
                let v = $(".n-msg").text();
                if (v){
                    Toastr.error('表单验证未通过：' + v)
                } else {
                    $("#menuSaveSyn").addClass('disabled');
                    $.post("weixin/menus/sync", {menus: JSON.stringify(getMenuList())}, function (ret) {
                        var msg = ret.hasOwnProperty("msg") && ret.msg != "" ? ret.msg : "";
                        if (ret.code == 1) {
                            Backend.api.toastr.success(ret.msg);
                            setTimeout("location.reload()", 1500); // 刷新
                        ...
                        $("#menuSaveSyn").removeClass('disabled');
                    }, 'json');
                }
            });
            ...
            $(document).on('click', "#getMenu", function () {
                $.post("weixin/menus/index", {}, function (ret) {
                    var msg = ret.hasOwnProperty("msg") && ret.msg != "" ? ret.msg : "";
                    if (ret.code == 1) {
                        Backend.api.toastr.success(ret.msg);
                        setTimeout("location.reload()", 1500); // 刷新
